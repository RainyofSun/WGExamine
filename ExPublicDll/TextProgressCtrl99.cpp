...
CTextProgressCtrl::CTextProgressCtrl()
{
    ...
    m_nBarWidth = -1;
	m_nMarkPos  = m_nPos;//add
	m_bShowMark = FALSE;//add
	m_nMarkBarWidth = -1;//add
}
...
void CTextProgressCtrl::OnPaint() 
{
    ...
    double Fraction = ((m_nPos - m_nMin) / (m_nMax - m_nMin));
	...
	LeftRect.top = LeftRect.bottom + (int)((LeftRect.top - LeftRect.bottom)*Fraction + 0.5);
    dc.FillSolidRect(LeftRect, m_colFore);
	...
	if (m_bShowMark)
	{   
		CRect MarkRect = ClientRect;
		double MarkFraction = ((m_nMarkPos - m_nMin) / (m_nMax - m_nMin));

		MarkRect.bottom = MarkRect.bottom - (int)((MarkRect.bottom - MarkRect.top)*MarkFraction + 0.5)+2;
		
		if (MarkRect.bottom >= 4)
		{
			MarkRect.top = MarkRect.bottom - 4;
		}
		else
		{
            MarkRect.top = 0;
		...
		dc.FillSolidRect(MarkRect, m_colMark);
	}
...
void CTextProgressCtrl::SetRange(double nLower, double nUpper)
{
    m_nMax = nUpper;
    m_nMin = nLower;
}

double CTextProgressCtrl::SetPos(double nPos) 
{    
    if (!::IsWindow(m_hWnd))
        return -1;
	if (m_nMin >= m_nMax) 
 ...
 /*   int nBarWidth = (int) (Fraction * rect.Width());*/
	...
	int nBarWidth = (int) (Fraction * rect.Height());
...
double CTextProgressCtrl::StepIt() 
{    
   return SetPos(m_nPos + m_nStepSize);
}

double CTextProgressCtrl::OffsetPos(double nPos)
{
    return SetPos(m_nPos + nPos);
}

double CTextProgressCtrl::SetStep(double nStep)
{
    double nOldStep = nStep;
    m_nStepSize = nStep;
    return nOldStep;
} 

void  CTextProgressCtrl::SetMarkShow(BOOL bShowMark)//add
{
	m_bShowMark = bShowMark;
}

double   CTextProgressCtrl::SetMarkPos(double nMarkPos)//add
{
	if (!::IsWindow(m_hWnd))
    return -1;

    double nOldPos = m_nMarkPos;
    m_nMarkPos = nMarkPos;

    CRect rect;
    GetClientRect(rect);

    double Fraction = ((m_nMarkPos - m_nMin) / (m_nMax - m_nMin));
 /*   int nBarWidth = (int) (Fraction * rect.Width());*/

	int nBarWidth = (int) (Fraction * rect.Height());
    if (nBarWidth != m_nMarkBarWidth)
    {
        m_nMarkBarWidth = nBarWidth;
        RedrawWindow();
    }
...
void  CTextProgressCtrl::SetMarkColour(COLORREF col)//add
{
	m_colMark = col;
}

COLORREF  CTextProgressCtrl::GetMarkColour()//add
{
	return m_colBk;
}
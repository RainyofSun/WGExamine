...
void CTextProgressCtrl::OnPaint() 
{
	...
	{   
		...
		double MarkFraction = (double)(m_nMarkPos - m_nMin) / ((double)(m_nMax - m_nMin));
    ...
    if (m_bShowText)
    {
        ...
        if (m_strText.GetLength())
           // str = m_strText;
		    str.Format("%s  %d%%", m_strText,(int)(Fraction*100.0));
        else
...
int   CTextProgressCtrl::SetMarkPos(int nMarkPos)//add
{
	if (!::IsWindow(m_hWnd))
    return -1;

    int nOldPos = m_nMarkPos;
    m_nMarkPos = nMarkPos;

    CRect rect;
    GetClientRect(rect);

    double Fraction = (double)(m_nMarkPos - m_nMin) / ((double)(m_nMax - m_nMin));
 /*   int nBarWidth = (int) (Fraction * rect.Width());*/
...
void CTextProgressCtrl::SetText(LPCTSTR lpStr)
{
	m_strText = (CString)lpStr;

}
void CTextProgressCtrl::SetShowHorizontalType(BOOL bHorizontal)
{
	m_bShowHorizontal = bHorizontal;
}

...
BEGIN_MESSAGE_MAP(CXULButton, CButton)
    //{{AFX_MSG_MAP(CXULButton)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CXULButton::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CXULButton::PreSubclassWindow() 
{
	UINT nBS;
...
BOOL CXULButton::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
HBRUSH CXULButton::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void CXULButton::OnSysColorChange() 
{
...
LRESULT CXULButton::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CXULButton::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CXULButton::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck
...
#ifdef	BTNST_USE_BCMENU
void CXULButton::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	BOOL bSetFlag = FALSE;
...
void CXULButton::OnEnable(BOOL bEnable) 
{
...
void CXULButton::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CXULButton::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CXULButton::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CXULButton::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CXULButton::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CXULButton::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CXULButton::OnClicked() 
{	
	SetFocus();
...
void CXULButton::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
...
void CXULButton::PaintBk(CDC* pDC)
{
...
HBITMAP CXULButton::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CXULButton::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CXULButton::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CXULButton::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
void CXULButton::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
...
//
HICON CXULButton::CreateGrayscaleIcon(HICON hIcon)
{
...
//
DWORD CXULButton::SetIcon(int nIconIn, int nIconOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
	...
	if(!hIconIn)
	{
		hInstResource = GetModuleHandle("IconDll.DLL");
		hIconIn = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nIconIn), IMAGE_ICON, 0, 0, 0);
...
//			Failed loading the specified resource.
//
DWORD CXULButton::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CXULButton::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//			Failed creating mask bitmap.
//
DWORD CXULButton::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
...
//				By default, CXULButton buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CXULButton::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CXULButton buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CXULButton::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//				By default, CXULButton buttons have BTNST_PRESSED_LEFTRIGHT style.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CXULButton::SetPressedStyle(BYTE byStyle, BOOL bRepaint)
{
	switch (byStyle)
	{
...
//			Function executed successfully.
//
DWORD CXULButton::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CXULButton::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CXULButton::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CXULButton::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CXULButton::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CXULButton::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CXULButton::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CXULButton::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CXULButton::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
void CXULButton::InitToolTip()
{
	if (m_ToolTip.m_hWnd == NULL)
	{
		...
		// Enable multiline
		m_ToolTip.SendMessage(TTM_SETMAXTIPWIDTH, 0, 200);
	} // if
} // End of InitToolTip
...
//
void CXULButton::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CXULButton::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CXULButton::ActivateTooltip(BOOL bActivate)
{
...
//				If TRUE the control will be repainted.
//
void CXULButton::DrawTransparent(BOOL bRepaint)
{
...
DWORD CXULButton::SetBk(CDC* pDC)
{
...
//			Function executed successfully.
//
DWORD CXULButton::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CXULButton::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CXULButton::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
...
//			Function executed successfully.
//
DWORD CXULButton::SetMenuCallback(HWND hWnd, UINT nMessage, LPARAM lParam)
{
...
//
#ifdef	BTNST_USE_SOUND
DWORD CXULButton::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
...
// This is a virtual function that can be rewritten in CXULButton-derived classes
...
//			Function executed successfully.
//
DWORD CXULButton::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	COLORREF	crColor;

	if (m_bMouseOnButton || m_bIsPressed)
		crColor =  RGB(100,200,100);//m_crColors[BTNST_COLOR_BK_IN];
	else
	{
		if (m_bIsFocused)
			crColor = m_crColors[BTNST_COLOR_BK_FOCUS];//RGB(128,255,128);
		else
...
//			Function executed successfully.
//
DWORD CXULButton::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	if (m_bIsPressed)
